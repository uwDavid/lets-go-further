...
func (app *application) readJSON(w http.ResponseWriter, r *http.Request, dst interface{}) error {
	// Implement additional limits on POST requests
	maxBytes := 1_048_576
	r.Body = http.MaxBytesReader(w, r.Body, int64(maxBytes))

	// Initiallize json.Decoder and decode the request body into the target destination.
	// err := json.NewDecoder(r.Body).Decode(dst)

	// initialize json.decoder and call DisallowUnknownFields() before decoding
	// decoder will return error instead of ignoring unmapped fields
...
// background() helper accepts an arbitrary func as a parameter
